...
with tempfile.TemporaryDirectory(prefix="orchard-pty-") as td:
    ...
    try:
        ...
        terminal.send(b" ")
        terminal.expect_screen("FULL TREEMAP")
        terminal.send(b" ")
        terminal.expect_screen("page 2")
        terminal.send(b"b")
        terminal.expect_screen("page 1")
        terminal.send(b"b")
        terminal.expect_screen("TREEMAP")
        terminal.send(b".\x0c")
        ...
        terminal.send(b"f")
        terminal.expect("RECURSIVE SEARCH")
        terminal.send(b"viewer.log")
        terminal.expect_screen("1 matches")
        terminal.send(b"\r")
        terminal.send(b"v")
        terminal.expect("FILE VIEW")
        terminal.expect("viewer-line-000")
        terminal.send(b" ")
        terminal.expect_screen("viewer-line-024")
        terminal.send(b"b")
        terminal.expect_screen("viewer-line-000")
        terminal.send(b"\x1b")
        terminal.read(0.5)
        terminal.send(b"t")
        terminal.expect("FILE VIEW")
        terminal.expect("viewer-line-079")
        with log.open("a") as stream:
            stream.write("viewer-line-080\n")
        terminal.send(b"r")
        terminal.expect_screen("viewer-line-080")
        terminal.send(b"\x1b[A")
        terminal.expect_screen("viewer-line-056")
        terminal.send(b"q")
        terminal.expect_screen("TREEMAP")
...
try:
    terminal.expect("Choose a volume")
    terminal.expect("Enter/click explore")
    terminal.expect_screen("mounted + unmounted")
    terminal.send(b"u")
    terminal.expect_screen("u show unmounted")
finally:
    terminal.close()
